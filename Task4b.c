...
******************************************************************************
* @file : main.c
* @brief : Task 4 - Scalability Test (STM32F0 - Memory Limited)
******************************************************************************
*/
...
// Scalability test: reduced sizes for STM32F0's limited RAM
typedef struct {
    uint32_t width;
    uint32_t height;
...
// Conservative test image sizes for STM32F0 (limited RAM)
ImageSize_t test_sizes[] = {
    {128, 128, "128x128 (Baseline)"},
    {160, 160, "160x160 (Small+)"},
    {192, 192, "192x192 (Medium-)"},
    {256, 256, "256x256 (Medium)"},
    {320, 240, "320x240 (QVGA)"},
    {400, 300, "400x300 (Small VGA)"},
    {512, 384, "512x384 (Half VGA)"}
    // Note: Larger sizes removed due to F0 RAM limitations
};

#define NUM_TEST_SIZES (sizeof(test_sizes)/sizeof(test_sizes[0]))

// Global variables for scalability results (reduced for F0)
...
// Chunking parameters for large images (more aggressive for F0)
#define MAX_CHUNK_PIXELS 20000  // Much smaller chunks for F0
...
int main(void)
{
    ...
    for (int i = 0; i < NUM_TEST_SIZES; i++) {
        ...
        if (total_pixels > MAX_CHUNK_PIXELS) {
            ...
            // Calculate chunk dimensions (more conservative for F0)
            uint32_t chunk_height = MAX_CHUNK_PIXELS / width;
            if (chunk_height == 0) chunk_height = 1;
            if (chunk_height > height) chunk_height = height;
            
            // Ensure chunk doesn't exceed available memory
            if (chunk_height > 100) chunk_height = 100; // Conservative limit for F0
            ...
            if (test_mode == 0) {
                ...
                // Note: Double precision is very slow on F0 (software floating point)
        ...
        // For larger images, give user feedback
    ...
    while (1)
    {
        // STM32F0 Scalability test results are stored in:
        // execution_time_ms[0-6]   - Execution time for each image size
        // pixels_per_second[0-6]   - Throughput for each image size
        // checksum[0-6]            - Verification checksums
        // memory_status[0-6]       - 0=Direct, 1=Chunked, 2=Failed
        // chunks_used[0-6]         - Number of chunks used for processing
        //
        // test_sizes[] contains the image dimensions and descriptions
        //
        // F0 specific notes:
        // - 48MHz clock (vs 120MHz F4)
        // - No FPU (software floating point)
        // - Limited RAM (~8KB for most F0 variants)
        // - More aggressive chunking required
        // - Smaller maximum image sizes tested
        //
        // Compare with F4 results to analyze:
        // - Clock speed impact (48MHz vs 120MHz)
        // - FPU vs software floating point performance
        // - Memory constraints on scalability
    }
}

/* Chunked Mandelbrot calculation for large images (F0 optimized) */
uint64_t calculate_mandelbrot_chunked(int width, int height, int max_iterations, int chunk_size) {
    ...
    for (int y_start = 0; y_start < height; y_start += chunk_size) {
        ...
        // Calculate this chunk using fixed-point arithmetic (optimized for F0)
        for (int y = y_start; y < y_start + chunk_height; y++) {
            for (int x = 0; x < width; x++) {
                // Use fixed-point arithmetic for better F0 performance
        ...
        // Flash LED to show progress on larger images
...
/* USER CODE BEGIN 4 */
// Mandelbrot using fixed point arithmetic (optimized for F0)
uint64_t calculate_mandelbrot_fixed_point_arithmetic(int width, int height, int max_iterations) {
